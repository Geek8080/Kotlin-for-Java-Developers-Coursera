...
fun TaxiPark.findFakeDrivers(): Set<Driver> =
        allDrivers.filter { driver -> driver !in  trips.map { trip -> trip.driver }}.toSet()
...
fun TaxiPark.findFrequentPassengers(driver: Driver): Set<Passenger> =
        allPassengers.filter { passenger -> trips.filter { trip -> trip.driver.equals(driver) && passenger in trip.passengers }.count() > 1 }.toSet()

/*
 * Task #4. Find the passengers who had a discount for majority of their trips.
 */
fun TaxiPark.findSmartPassengers(): Set<Passenger> =
        allPassengers.filter { passenger -> trips.filter { trip -> passenger in trip.passengers && trip.discount != null }.count() > trips.filter { trip -> passenger in trip.passengers && trip.discount == null }.count() }.toSet()
